...
def validate():
    ...
    assets = {kind: ROOT / f"projects/neoforge/build/libs/{name}-{kind}.jar" for kind in ("installer", "universal", "sources")}
    ...
    with ZipFile(assets["installer"]) as archive:
        ...
        for platform in ("unix", "win"):
            require(installed_version in archive.read(f"data/{platform}_args.txt").decode(), "Stale server arguments.")
        require(not any(n.startswith("maven/") and n != "maven/" + maven_path and not n.endswith("/") for n in archive.namelist()),
...
def main():
    ...
    if args.check:
        print(f"PASS: {name}: installer, embedded runtime, isolated paths, source identity, and archive contents.")
        return

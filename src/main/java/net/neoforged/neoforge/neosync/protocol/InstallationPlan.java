...
public final class InstallationPlan {
    ...
    public record File(SyncManifest.Artifact artifact, URI source, boolean available) {}
    ...
    public static InstallationPlan create(SyncEndpoint endpoint, byte[] bytes, Set<String> verifiedAvailable,
        ...
        for (var artifact : manifest.files()) {
            ...
            var source = artifact.sources().stream().filter(candidate -> candidate.type().equals("external")).findFirst()
                    .orElseThrow(() -> new IOException("No external HTTPS source is configured for " + artifact.fileName() + ". Server file hosting is not available yet."));
            URI url = externalSource(source.url());
            files.add(new File(artifact, url, verifiedAvailable.contains(artifact.sha256())));
        }
    ...
    public List<String> reviewLines() {
        ...
        for (var file : files) {
            ...
            lines.add("Source: " + file.source() + " (unverified)");
    ...
    public List<String> warningLines() {
        ...
        for (var file : files) lines.add(file.artifact().fileName() + " — " + file.source());
